    ...
    if (!file_exists('servers.txt')) {
        ...
        foreach ($servers as $server) {
            ...
            foreach ($files as $file_line) {
                ...
                // Strip the extension from the files.txt line (hash.ext) to get the pure hash
                $file_hash = pathinfo($file_line, PATHINFO_FILENAME);

                // Check if this hash is already linked to this server's host before
                // requesting anything else from it.
                ...
                $existing_hosts = [];
                if ($existing) {
                    $existing_hosts = json_decode($existing['hosts'], true);
                    if (!is_array($existing_hosts)) {
                        $existing_hosts = [];
                    }

                    if (in_array($base_url, $existing_hosts, true)) {
                        // Already indexed for this server - skip the JSON request entirely.
                        $skipped[] = "Skipped (already linked to " . htmlspecialchars($base_url) . "): " . htmlspecialchars($file_hash);
                        continue;
                    }
                }

                // Metadata lives at <server>/info/<hash>.json
                ...
                // No integrity check on the file itself - we trust the JSON metadata as-is.
                if ($data && isset($data['filename'], $data['size'], $data['extension'])) {

                    if ($existing) {
                        // Existing entries are never overwritten - only the hosts list is updated.
                        $existing_hosts[] = $base_url;
                        $update_stmt = $pdo->prepare("UPDATE file_registry SET hosts = ? WHERE file_hash = ?");
                        $update_stmt->execute([json_encode($existing_hosts), $file_hash]);
                        $updated++;
                    } else {
                        $initial_hosts = json_encode([$base_url]);
                        $file_date = !empty($data['date']) ? date('Y-m-d H:i:s', strtotime($data['date'])) : null;

                        $insert_stmt = $pdo->prepare("
                            INSERT INTO file_registry
                            (file_hash, server_filename, filename, file_size, extension, public_key, server_public_key, node_info, description, file_date, hosts)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ");
                        $insert_stmt->execute([
                            $file_hash,
        ...
        $skip_summary = !empty($skipped) ? "<br><small style='display:block; margin-top:10px; color:#475569;'><strong>Skipped (already indexed for this server):</strong><br>" . implode("<br>", $skipped) . "</small>" : "";
        ...
        $sync_message = "<div class='alert success'>Sync complete! Added $inserted new files. Updated host links for $updated existing files. $skip_summary $err_summary</div>";
    }
}
...
if ($search !== '') {
    $count_stmt = $pdo->prepare("SELECT COUNT(*) FROM file_registry WHERE filename LIKE ? OR description LIKE ? OR file_hash LIKE ?");
...
if ($search !== '') {
    $data_stmt = $pdo->prepare("SELECT * FROM file_registry WHERE filename LIKE ? OR description LIKE ? OR file_hash LIKE ? ORDER BY indexed_at DESC LIMIT ? OFFSET ?");
        ...
        ?>
            <div class="card">
                <div class="card-header">
                    <h3 class="file-title"><?= htmlspecialchars($row['filename']) ?></h3>
                ...
                <div class="details-grid">
                    ...
                    <div><strong>File Date:</strong> <?= htmlspecialchars($row['file_date'] ?: 'Unknown') ?></div>
                    ...
                    <div><strong>Server Public Key:</strong> <?= !empty($row['server_public_key']) ? htmlspecialchars(substr($row['server_public_key'], 0, 20)) . '...' : 'None' ?></div>
