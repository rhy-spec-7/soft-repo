...
public class ServersDiscovery {
    ...
    private static CheckResult checkServer(String baseUrl) {
        ...
        // Read the body now, while the connection is still open
        ...
        try (BufferedReader reader = new BufferedReader(
                ...
                c.disconnect();
                r.online = false;
                r.reason = "failed to read /files.txt: " + e.getMessage();
                return r;
        } finally {
                c.disconnect();
        }
        // 2. Random file test (or pass-through)
        if (!remoteFiles.isEmpty()) {
                r.online = true;
                r.testPassed = true;
                r.reason = "Server is online (integrity not checked)";
                return r;
        }
        String filename = remoteFiles.get(random.nextInt(remoteFiles.size()));
